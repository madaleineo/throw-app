...
export default function Groups() {
  ...
  return (
    <div className="w-screen h-screen flex flex-row justify-center p-4 bg-burnt-orange">
      {/* Create Group Section */}
      <div className="flex flex-col items-center w-80 px-24">
        ...
        <Form className="flex flex-col" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="py-2 px-8 bg-slate-400 border border-black rounded hover:bg-slate-500 transition"
          >
            Create Group
          </button>
        </Form>
      ...
      <div className="flex flex-col items-center w-80 px-24">
        ...
        <div className="flex flex-col gap-4">
          {data.groups.map(groups => (
            <div key={groups.id}>
              <h3 className="text-lg font-semibold">{groups.name}</h3>
            </div>
          ))}
        </div>
      </div>
